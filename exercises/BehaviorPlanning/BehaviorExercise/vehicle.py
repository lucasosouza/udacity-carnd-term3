class Vehicle(object):
  ...
  def update_state(self, predictions):
    ...
    # if it was preparing to change lanes, do it now
    if self.state == "PLCL":
      self.state = "LCL"
    elif self.state == "PLCR":
      self.state = "LCR"
    # else, I will decide if I will change lane left or right
    else:
      # basic strategy, go straight
      self.state = "KL"
      # check positions in next 5 timeframes
      for t in range(5):
        l,   s,   v,   a = self.state_at(t)
        for car, preds in predictions.items():
          # if they will likely crash, prepare to move left
          if preds[t]["s"] <= (s-1) and preds[t]["lane"] == self.lane:
            # at leftmost lane, pass on the right lane
            if l == 3:
              self.state = "PLCR"
            # else use the left lane
            else:
              self.state = "PLCL"
