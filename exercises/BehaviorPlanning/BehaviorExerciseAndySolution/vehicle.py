...
class Vehicle(object):
  ...
  def _trajectory_for_state(self,state,predictions, horizon=5):
    ...
    for i in range(horizon):
      ...
      # restore state to initial snapshot
      # what is the point of restoring the state after each i in horizon?
      self.restore_state_from_snapshot(snapshot)
      self.state = state

      # adjust to new proposed state
      self.realize_state(predictions)

      # assert lane is whithin boundaries of the road
      assert 0 <= self.lane < self.lanes_available, "{}".format(self.lane)

      # increases s and d based on speed
      self.increment()
    ...
    # restore state from snapshot, after simulations
    self.restore_state_from_snapshot(snapshot)
  ...
  def increment(self, dt=1):
    ...
    # increases s position based on velocity and time change
    self.s += self.v * dt

    # increases velocity based on acceleration and time change
    self.v += self.a * dt
